...
class GameBoard extends React.Component {
  ...
  componentDidMount = () => {
    ...
    this.eventsChildAddedListener = eventsRef.limitToLast(50).on(
      'child_added',
      (eventSnap) => {
    ...
    this.winsValueListener = winsRef.on(
      'value',
      (winsSnap) => {
        this.setState({
          wins: winsSnap.val(),
    ...
    this.gitHubChildAddedListener = loggedInUsersRef.child('github').on(
      'child_added',
      () => {
      ...
      (error) =>
        setErrorMessage(
          `Failed to subscribe to GitHub logged in users (child_added) in Firebase: ${error.message}`
        )
    );
    this.gitHubChildRemovedListener = loggedInUsersRef.child('github').on(
      'child_removed',
      () => {
        this.setState(({loggedInUserCounts}) => ({
          loggedInUserCounts: {
      ...
      (error) =>
        setErrorMessage(
          `Failed to subscribe to GitHub logged in users (child_removed) in Firebase: ${error.message}`
        )
    );
    this.twitterChildAddedListener = loggedInUsersRef.child('twitter').on(
      'child_added',
      () => {
        this.setState(({loggedInUserCounts}) => ({
          loggedInUserCounts: {
      ...
      (error) =>
        setErrorMessage(
          `Failed to subscribe to Twitter logged in users (child_added) in Firebase: ${error.message}`
        )
    );
    this.twitterChildRemovedListener = loggedInUsersRef.child('twitter').on(
      'child_removed',
      () => {
        this.setState(({loggedInUserCounts}) => ({
          loggedInUserCounts: {
      ...
      (error) =>
        setErrorMessage(
          `Failed to subscribe to Twitter logged in users (child_removed) in Firebase: ${error.message}`
        )
    );
  };
